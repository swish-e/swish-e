...
and the examples in the C<filters> and C<filter-bin> directory for more information.

The FileFilter directive can be used in any of the input methods, but the -S http method
(spidering with the SwishSpider program) only indexes documents of content-type
text/* (e.g. text/plain, text/html, text/xml, ...) so filter programs can not
be used to convert documents such as PDF files.
