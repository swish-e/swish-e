...
would call the doc2txt.pm perl module, and a document of type
"appliation/pdf" would use the pdf2html.pm module.  The F<prog-bin/SwishSpiderConfig.pl> file
shows this usage.
...
The F<prog-bin/SwishSpiderConfig.pl> also shows an example of using the SWISH::Filter module for filtering.
All that is required is setting Perl's @INC array to point to the directory that contains the SWISH::Filter module
...
use the SWISH::Filter module.  Therefore, all that's required to use this system with -S http is setting
...
See the F<prog-bin/SwishSpiderConfig.pl> file for an example of using the SWISH::Filter
module.
...
the F<prog-bin/SwishSpiderConfig.pl> file.
...
The -e feature uses many tempory files (something like 377) plus the index files
