...
the GNU Auto Tools.  The significant changes are where files are were files
are installed, and the use of Libtool to create the swish-e library as a
shared library on many platforms.
...
Documentation is provided in the Swish-e distribution package in two forms,
...
directory, of course.  The HTML documentation is generated from the POD
files.

Patches or updates to the documentation should be done against the POD files 
located in the pod directory.
