...
L<DontBumpPositionOnMetaTags|/"item_DontBumpPositionOnMetaTags"> *list of names*

=item *

L<EnableAltaVistaSyntax|/"item_EnableAltaVistaSyntax">  [yes|NO]
...
B<The following items are currently not available until Swish can parse a configuration
file while searching>

=over 4

=item EnableAltaVistaSyntax [yes|NO]
    
Allows the usage of a basic Altavista(c) like search syntax.
This means a search query can contain "+" and "-" as syntax parameter.

Example:

    swish-e -w "+word1 +word2 -word3"
    "+"  = following word has to be in document
    "-"  = following word may not be in document
...
=item IndexContents [TXT|HTML|XML|LST]  *file extensions*

Swish knows how to parse TXT, HTML, and XML documents.  This directive maps file extensions to
those types of parsers.  LST are special multiple document XML files, described below.
...
LST files are XML files that contain multiple documents.  The documents are separated by the first XML
tag found in the document (each time that tag is found it is considered a new document).
...
When reporting results from a query, swish will still return the document name, but will also return
a document offset in the property C<swishstartpos>, and the length of the sub-document in C<swishdocsize>
(these properties are available by using the C<-x> format option).

For example, you could have text files that contain SQL queries, and the queries might generate quite a
number of results (documents) from a database.  You can instruct swish to "index" these files, and use a
filter to convert the SQL queries into documents.  In other words, indexes a file, but a swish filter converts
that file (which contains the SQL query statement) into a query and returns, perhaps, many documents.
...
The mysqlfilter.sh program should read the *.sql file, proccess the query/select, and 
format the output in the "LST" style:
...
Care must be taken when returning multiple document files to swish, as swish will load all data
into memory for each file.  In other words, don't try to index thousands of documents as a single
LST type of document.

B<Note:> Some of this may be changed in the future to use content-types.  See L<SWISH-3.0|SWISH-3.0>
...
ASCII I<entities> can be converted automatically while indexing documents of type HTML and XML.
...
C</path/to/config hello there> as three command line arguments to the program.
...
C<IgnoreMetaTags> defines a list of metanames to ignore while indexing XML files.
This is useful to avoid indexing specific data from a file.  For example:
...
Warning: Any data listed in C<IgnoreMetaTags> will not be indexed.  
...
This is done after converting HTML entities and splitting the input into words based on B<WordCharacters>.
...
The characters you are converting B<from> and B<to> (even with C<:ascii7:>) must be listed in C<WordCharacters>
(and C<BeginCharacters>/C<EndCharacters>).
...
=item DontBumpPositionOnMetaTags *list of names*

Since metatags are typically separate data fields, the word position counter is automatically bumped
between metatags.  This prevents matching a phrase that spans more than one metaname.
C<DontBumpPositionOnMetaTags> disables this feature for the listed metanames.
...
but this phrase search will faile
...
Here are some simple examples of filters.
