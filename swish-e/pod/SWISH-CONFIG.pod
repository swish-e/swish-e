...
use Swish-e's internal haml parser.  The C<DefaultContents> directive may be
...
This system is designed to work the the -S http and -prog methods, but may also be used
with the C<FileFilter> feature and -S fs indexing method.  See F<filter-bin/swish_filter.pl> for
and example.
