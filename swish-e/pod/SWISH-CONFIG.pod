...
=item FileFilterMatch   *filter-prog*   *filter-options*  *regular expression(s)*

This is simliar to C<FileMatch> except uses regular expressions to match against the file name.
The *filter-prog* should be the path the the program.  Unlike C<FileFilter> this does B<not> use
...
Or to specify more than one extension:
...
You may also preceed an expression with "not" to negate regular expression that follows.
