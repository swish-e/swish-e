...
A number of improvements in indexing and searching speed were made, especially for searching
...
The search parser was rewritten to correct a number of logic errors.  Previously, swish
...
setting, and that it could be stemmed.  This has been fixed.
...
The builtin list of stop words has been removed. Use of the SwishDefault word
will generate a warning. A sample file C<stopwords.txt> has been included if
you wish to use these default words using C<IgnoreWords File: stopwords.txt>
...
Swish can not currently parse date strings.
