...
directory.  For example, the Win32 files can be found in C<src/win32>, and instructions
for building under VMS can be found in C<src/vms>.

The Windows binary is distributed as a separate package from the source distribution.
See http://swish-e.org for download information.

Note that some commands (FileFilter, -S prog directives) need to be modified when using Windows.
