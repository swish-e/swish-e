...
See L<QUESTIONS AND TROUBLESHOOTING|QUESTIONS AND TROUBLESHOOTING>.
...
Swish-e is written in C and up to this time has been tested on
...
Libxml2 is very stronly recommended.  It is used for parsing both HTML and
...
reecommended for use with Swish-e.  Zlib is used for compressing text stored
...
administrator or visit http://search.cpan.org.  Every Perl installation
should have the LWP library installed.  
...
This module is not requires, but if installed can be helpful in filtering
...
These may be installed after swish-e is installed.
...
The "$" symbol indicates steps run as an unprivilaged user.  The "#" 
...
Here's another example.  This might be used if you do not have root access
or to install swish someplace other than /usr/local.
...
but requires GNU Make.  Without GNU Make you will likey need to build
...
needed on some platforms (Solaris 2.6 perhaps).
    ...
    $ make >/dev/null  (only see warnings and errors)
    $ make check
    ...
    ==================
    All 3 tests passed
...
$prefix/lib/swish-e/perl to the PERL5LIB environment varilable.
...
will index I<all> files in the /var/www/htdocs directory.  You may also one or more 
files or directories with the -i options.  By default this will create an index (which is 
made up of more than one file) in the current directory called F<index.swish-e>.

Then to search the resulting index for a give word:

     swish-e -w apache

Swish assumes files are HTML unless otherwise told so.  So if /var/www/htdocs contains
non-HTML then you will need a configuration file to limit the files that Swish-e indexes. 
Create a file called "swish.conf":
...
page.  There are many possible settings.  Good advice is to use as few settings as possible 
when first starting out with Swish-e.

The runtime options (switches) are described in the L<SWISH-RUN|SWISH-RUN> manual page.  You 
may also see a summary of options by running:
...
This says to use the -S prog input source method.  Note that in this case the IndexDir
settings does not list a file or directory to index, but a program name run.  This program,
spider.pl, does the work of fetching the documents from the web server and passing them to
...
Running a script under Windows requires specifying the interptater (e.g. perl.exe) and then
use SwishPropParameters to specify the script and the script's parameters.
See I<Notes when using -S prog on MS Windows> in L<SWISH-RUN|SWISH-RUN>.
...
IndexDir needs to be an absolute path.
...
In additition to the above, there's two key concepts that you need to be familiar with: 
...
it lists the pages (or documents) where that word can be found.

Swish-e can create multiple index tables within the same index file.  For example, you
might want to create an index of just words in HTML titles in addition to an index of the
words in the body of the document.  Some database systems might call these different
"fields" or "columns", but swish-e calls them I<MetaNames> (as a result of first indexing
HTML meta tags).
...
The Metaname "swishdefault" is the name used by Swish-e if no other name is specified.
The following two searches are the same:
...
Swish-e search results is a list of files -- actually file numbers.  Data can be associated 
...
This example demonstrates how to spider a web site and setup a CGI script to
provide a web-based search page using Perl programs included in the Swish-e distribution:
F<spider.pl> will be used for reading files from the web server, and F<swish.cgi> will 
provide the web search form and display results.

As an example we will index the Apache Web Server documenation installed on the local
computer at http://localhost/apache_docs/index.html

=over 4

=item 1: Create a Working Directory

Create a directory to store the Swihs-e configuration and the Swish-e
index.
...
=item 2: Create a Swish-e Configuration file
    ...
    # Swish-e config to index the Apache documenation
    #
...
Finally, verify that the index can be searched:
...
But the -t feature will not be used in this example.


=item 4: Setup the CGI script

Swish-e does not include a web server, therefore you must use your own web server.
...
This example will use a site cgi-bin directory locate at /usr/lib/cgi-bin. 
...
library.  Any web browser can run this test.
...
occurr.  The message "Service currently unavailable" reported by running swish.cgi
