...
if you are using C<StoreDescription|SWISH-CONFIG/"item_StoreDescription">
...
As mentioned above, the XML parser that included with swish with is fine.  The HTML parser in
swish has been in use for years, but the parser provided by libxml2 is preferred.
The libxml2 HTML parser offers more features, and is more accurate.
...
If you do not have root access and/or did not install libxml2 in the standard locations (for header
and library files), then specify the path where libxml2 was installed:

    ./configure --with-libxml2=$HOME/libxml2-2.4.5
    
You may need to set LD_RUN_PATH or LDFLAGS when building swish to set the
location of the libxml2 shared library:

    LDFLAGS="-R$HOME/libxml2-2.4.5/.libs -L$HOME/libxml2-2.4.5/.libs" \
    ./configure --with-libxml2=$HOME/libxml2-2.4.5

Now, you run C<make> as described in the following section.

libxml2 can be downloaded from http://www.xmlsoft.org.
...
B<Building Swish with zlib>

Swich can be built with zlib.  Building with zlib is similar to the instructions
for building zlib with libxml2 above.  To select support for zlib run configure
with:
