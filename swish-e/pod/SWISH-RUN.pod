...
"Update" will update the index with the given file if the date of the record is
...
"Index" will add the file to the index, not checking to see if the file
already exists.

If this header is not specified then it default to the mode specified on
