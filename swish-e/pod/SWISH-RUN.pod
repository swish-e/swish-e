...
B<Format of "formatstring":>
    ...
    "text<propertyname>text<propertyname fmt=subfmt>text..."
...
B<Sub-formats of properties:>

Properties listed in an C<-x> format string can include format control strings.
These "sub-formats" are used to control how the contents of the associated property are printed.
Sub-formats are used like C-language printf formats.
The sub-format is specified by including the attribute "fmt" within the property tag.

General syntax:

    -x '<propertyname fmt="subfmt">'

where C<subfmt> controls the output format of C<propertyname>.    

Examples of sub-format strings:
...
B<Rainer -- are strftime and sprintf formats fully implemented?>

The first character of a subfmt string defines the delimiter for the sub-format string.
For example,
...
B<Text in "formatstring" or "subfmt":>

Text will be output as-is in format strings (and sub-format strings).
...
=item -X[0|1|2|3|<n>]  (extended header output)

The C<-Xn> switch generates extened I<header> output.  This is most useful when searching more than one
index file at a time, either by specifying more than one index file with the C<-f> switch, or when searching
a merged index file.  In these cases, -X will generate a combined set of headers followed by a set of
headers specific to each index file.  This gives access to the settings used to generate each index file.

Even when searching a single index file, C<-Xn> will provided additional information about the index file,
how it was indexed, and how swish is interperting the query.

    -X0 :  print no header information, output only search result entries.
    -X1 :  print standard result header (default).
    -X2 :  print add. header information for each searched index file.
    -X3 :  enhance header output.
    -X9 :  print add. diagnostic information in the header of the results (changed from: C<-v 4>)
