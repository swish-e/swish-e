...
running under Unix.  Under Windows you will may not need to use quotes, but you will need to
backslash the quotes used to delimit phrases:

    swihs-e -w \"a phrase\"

The search may be limited to a I<MetaName>.  For example:
...
The string C<dq> means "double-quotes".  Not sure why it's needed.
B<How about entering '\t' or 'tab' for a tab?>
...
You can specify multiple files to decode.  Use with C<-v 4> for even more output.
