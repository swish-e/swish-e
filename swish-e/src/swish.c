...
int     main(int argc, char **argv)
{
    ...
    structure = 1;
    if (!lenwordlist)
        wordlist = (char *) emalloc((lenwordlist = MAXSTRLEN) + 1);
    wordlist[0] = '\0';
    if (!lenstructstr)
        structstr = (char *) emalloc((lenstructstr = MAXSTRLEN) + 1);
    structstr[0] = '\0';
    setlocale(LC_CTYPE, "");
