...
B<Jose, can the perl/Makefile be setup to look in ../src for libswish-e.a? so this step isn't required?>
...
See L<SWISH-INDEX|SWISH-INDEX> and L<SWISH-SEARCH|SWISH-SEARCH> for complete documentation of the various indexing
...
currently (B<why?>) each comment or directive must begin in column 1.
The complete list of all configuration file directives are described in L<SWISH-INDEX|SWISH-INDEX>.
...
contain the word B<sample>:
    ...
    ... (a number of other headers snipped)
    #
    # Number of hits: 2
...
To search for a phrase in a document use double-quotes to delimit your search term.  (The phrase
delimiter is set in F<src/swish.h>.)

For example:

    ../src/swish-e -w '"This is sample"'

    # Number of hits: 1
    1000 ./test.html "If you are seeing this, the test was successful!" 437

Note that the double quotes must be protected from the shell by using single quotes.

B<** Single quotes don't work on DOS, right David?>
...
Again, please see L<SWISH-INDEX|SWISH-INDEX> and L<SWISH-SEARCH|SWISH-SEARCH> for complete
...
Please search the SWISH-E list archive before posting, and check the L<SWISH-FAQ> to
