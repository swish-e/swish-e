...
Comments and directives currently must start at column 1 in the configuration file.  Comments are any line
that beging with a "#".
...
directives. (The directives listed below are grouped in this order).  You may also split your directives up into different configuration files and specify more than
one configuration file when running swish-e from the command line.
...
How do I get pod2html to create Name tags for items below?
...
DefaultContents [TXT|HTML|XML]  *file extensions*
...
B<Note:> This may be changed in the future to use content-types.  See L<SWISH-PROPOSAL|SWISH-PROPOSAL>


=item DefaultContents [TXT|HTML|XML]

This sets the default parser for documents that are not specified in B<IndexContents>
...
=item OkNoMeta [yes|no]

If B<OkNoMeta> is set to C<yes> (the default) then any META tag found in the document that is not defined
in the B<MetaNames> directive will be indexed, but the META tag contents will be added to the I<main index> and
not to any meta name.  In other words, the words of the META tag will be indexed as if the words were part of the
body of the document.  If B<OkNoMeta> is set to C<no> then SWISH-E will halt indexing with an error if a META tag
is detected that is not listed in the MetaNames directive.  This can be helpful to catch misspelled Meta names
or names that you did not anticipate.

=item ReqMetaName [yes|no]

The above behavior of OkNoMeta can be modified by the B<ReqMetaName> directive.
If this setting is defined as C<yes> then any META tag found in the document that is I<not>
listed in MetaNames will simply be ignored and not indexed.  This is helpful if you have meta data in your source
documents that you do not want to index.  The default is C<no>, so the entire document is indexed.

B<Check -- this work the same for XML?>
...
You may specify external programs to be used for filtering documents.  For example, if all your
...
B<FileFilter> directive>.  SWISH-E calls the external program passing the as arguments the file path
that swish is about to index, and optionally, the URL from the http file source method.

The filter program must open the file, process its contents, and return it to SWISH-E by printing to STDOUT.

Note that this can add a significant of time to the indexing process.  If you have many files to filter you
should consider writing your filter in C instead of a shell or perl script.

B<Note:> Filtering in swish may change in the future (see L<SWISH-PROPOSAL|SWISH-PROPOSAL>).

=over 4

=item FilterDir  *path-to-directory*

This is the path to a directory where filters are stored.
