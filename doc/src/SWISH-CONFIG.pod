...
L<IndexComments|/"item_IndexComments"> [0|1]
...
L<OkNoMeta|/"item_OkNoMeta"> [yes|no]
...
L<ReqMetaName|/"item_ReqMetaName"> [yes|no]
...
If B<OkNoMeta> is set to C<yes> (the default) then any meta tag contents found in the document that is not defined
in the B<MetaNames> directive will be indexed, but the tag contents will be added to the I<main index> and
not to any meta name.  In other words, the words of the meta tag will be indexed as if the words were part of the
body of the document, and not within a meta tag.  If B<OkNoMeta> is set to C<no> then SWISH-E will halt indexing with an error if a META tag
...
and is especially useful when the source documents are no longer available.

SWISH-E can also use the PropertyNames for sorting the search results (see the C<-s> switch).
...
B<XXX FIX -- 2.1.10-devel6 still required the &>
...
B<FileFilter> directive.  SWISH-E calls the external program passing as arguments the file path
