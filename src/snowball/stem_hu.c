...
extern "C" {
#endif
extern SWINT_T hungarian_ISO_8859_1_stem(struct SN_env * z);
#ifdef __cplusplus
}
#endif
static SWINT_T r_double(struct SN_env * z);
static SWINT_T r_undouble(struct SN_env * z);
static SWINT_T r_factive(struct SN_env * z);
static SWINT_T r_instrum(struct SN_env * z);
static SWINT_T r_plur_owner(struct SN_env * z);
static SWINT_T r_sing_owner(struct SN_env * z);
static SWINT_T r_owned(struct SN_env * z);
static SWINT_T r_plural(struct SN_env * z);
static SWINT_T r_case_other(struct SN_env * z);
static SWINT_T r_case_special(struct SN_env * z);
static SWINT_T r_case(struct SN_env * z);
static SWINT_T r_v_ending(struct SN_env * z);
static SWINT_T r_R1(struct SN_env * z);
static SWINT_T r_mark_regions(struct SN_env * z);
#ifdef __cplusplus
extern "C" {
#endif
...
static SWINT_T r_mark_regions(struct SN_env * z) {
    ...
    lab1:
        ...
        {    /* gopast */ /* grouping v, line 53 */
            SWINT_T ret = out_grouping(z, g_v, 97, 252, 1);
            if (ret < 0) return 0;
            z->c += ret;
        }
...
static SWINT_T r_v_ending(struct SN_env * z) {
    ...
    switch(among_var) {
        case 0: return 0;
        case 1:
            {   SWINT_T ret = slice_from_s(z, 1, s_0); /* <-, line 62 */
                if (ret < 0) return ret;
            }
            break;
        case 2:
            {   SWINT_T ret = slice_from_s(z, 1, s_1); /* <-, line 63 */
                if (ret < 0) return ret;
            }
            break;
    }
...
static SWINT_T r_double(struct SN_env * z) {
    {   SWINT_T m_test = z->l - z->c; /* test, line 68 */
...
static SWINT_T r_undouble(struct SN_env * z) {
    ...
    z->bra = z->c; /* ], line 73 */
    {   SWINT_T ret = slice_del(z); /* delete, line 73 */
        if (ret < 0) return ret;
    }
    return 1;
}
...
static SWINT_T r_instrum(struct SN_env * z) {
    ...
    {   SWINT_T ret = slice_del(z); /* delete, line 81 */
        if (ret < 0) return ret;
    }
    {   SWINT_T ret = r_undouble(z);
...
static SWINT_T r_case(struct SN_env * z) {
    ...
    {   SWINT_T ret = slice_del(z); /* delete, line 111 */
        if (ret < 0) return ret;
    }
    {   SWINT_T ret = r_v_ending(z);
...
static SWINT_T r_case_special(struct SN_env * z) {
    ...
    switch(among_var) {
        case 0: return 0;
        case 1:
            {   SWINT_T ret = slice_from_s(z, 1, s_2); /* <-, line 117 */
                if (ret < 0) return ret;
            }
            break;
        case 2:
            {   SWINT_T ret = slice_from_s(z, 1, s_3); /* <-, line 118 */
                if (ret < 0) return ret;
            }
            break;
        case 3:
            {   SWINT_T ret = slice_from_s(z, 1, s_4); /* <-, line 119 */
                if (ret < 0) return ret;
            }
            break;
    }
...
static SWINT_T r_case_other(struct SN_env * z) {
    ...
    switch(among_var) {
        case 0: return 0;
        case 1:
            {   SWINT_T ret = slice_del(z); /* delete, line 125 */
                if (ret < 0) return ret;
            }
            break;
        case 2:
            {   SWINT_T ret = slice_del(z); /* delete, line 126 */
                if (ret < 0) return ret;
            }
            break;
        case 3:
            {   SWINT_T ret = slice_from_s(z, 1, s_5); /* <-, line 127 */
                if (ret < 0) return ret;
            }
            break;
        case 4:
            {   SWINT_T ret = slice_from_s(z, 1, s_6); /* <-, line 128 */
                if (ret < 0) return ret;
            }
            break;
    }
...
static SWINT_T r_factive(struct SN_env * z) {
    ...
    {   SWINT_T ret = slice_del(z); /* delete, line 137 */
        if (ret < 0) return ret;
    }
    {   SWINT_T ret = r_undouble(z);
...
static SWINT_T r_plural(struct SN_env * z) {
    ...
    switch(among_var) {
        case 0: return 0;
        case 1:
            {   SWINT_T ret = slice_from_s(z, 1, s_7); /* <-, line 143 */
                if (ret < 0) return ret;
            }
            break;
        case 2:
            {   SWINT_T ret = slice_from_s(z, 1, s_8); /* <-, line 144 */
                if (ret < 0) return ret;
            }
            break;
        case 3:
            {   SWINT_T ret = slice_del(z); /* delete, line 145 */
                if (ret < 0) return ret;
            }
            break;
        case 4:
            {   SWINT_T ret = slice_del(z); /* delete, line 146 */
                if (ret < 0) return ret;
            }
            break;
        case 5:
            {   SWINT_T ret = slice_del(z); /* delete, line 147 */
                if (ret < 0) return ret;
            }
            break;
        case 6:
            {   SWINT_T ret = slice_del(z); /* delete, line 148 */
                if (ret < 0) return ret;
            }
            break;
        case 7:
            {   SWINT_T ret = slice_del(z); /* delete, line 149 */
                if (ret < 0) return ret;
            }
            break;
    }
...
static SWINT_T r_owned(struct SN_env * z) {
    ...
    switch(among_var) {
        case 0: return 0;
        case 1:
            {   SWINT_T ret = slice_del(z); /* delete, line 155 */
                if (ret < 0) return ret;
            }
            break;
        case 2:
            {   SWINT_T ret = slice_from_s(z, 1, s_9); /* <-, line 156 */
                if (ret < 0) return ret;
            }
            break;
        case 3:
            {   SWINT_T ret = slice_from_s(z, 1, s_10); /* <-, line 157 */
                if (ret < 0) return ret;
            }
            break;
        case 4:
            {   SWINT_T ret = slice_del(z); /* delete, line 158 */
                if (ret < 0) return ret;
            }
            break;
        case 5:
            {   SWINT_T ret = slice_from_s(z, 1, s_11); /* <-, line 159 */
                if (ret < 0) return ret;
            }
            break;
        case 6:
            {   SWINT_T ret = slice_from_s(z, 1, s_12); /* <-, line 160 */
                if (ret < 0) return ret;
            }
            break;
        case 7:
            {   SWINT_T ret = slice_del(z); /* delete, line 161 */
                if (ret < 0) return ret;
            }
            break;
        case 8:
            {   SWINT_T ret = slice_from_s(z, 1, s_13); /* <-, line 162 */
                if (ret < 0) return ret;
            }
            break;
        case 9:
            {   SWINT_T ret = slice_del(z); /* delete, line 163 */
                if (ret < 0) return ret;
            }
            break;
    }
...
static SWINT_T r_sing_owner(struct SN_env * z) {
    ...
    switch(among_var) {
        case 0: return 0;
        case 1:
            {   SWINT_T ret = slice_del(z); /* delete, line 169 */
                if (ret < 0) return ret;
            }
            break;
        case 2:
            {   SWINT_T ret = slice_from_s(z, 1, s_14); /* <-, line 170 */
                if (ret < 0) return ret;
            }
            break;
        case 3:
            {   SWINT_T ret = slice_from_s(z, 1, s_15); /* <-, line 171 */
                if (ret < 0) return ret;
            }
            break;
        case 4:
            {   SWINT_T ret = slice_del(z); /* delete, line 172 */
                if (ret < 0) return ret;
            }
            break;
        case 5:
            {   SWINT_T ret = slice_from_s(z, 1, s_16); /* <-, line 173 */
                if (ret < 0) return ret;
            }
            break;
        case 6:
            {   SWINT_T ret = slice_from_s(z, 1, s_17); /* <-, line 174 */
                if (ret < 0) return ret;
            }
            break;
        case 7:
            {   SWINT_T ret = slice_del(z); /* delete, line 175 */
                if (ret < 0) return ret;
            }
            break;
        case 8:
            {   SWINT_T ret = slice_del(z); /* delete, line 176 */
                if (ret < 0) return ret;
            }
            break;
        case 9:
            {   SWINT_T ret = slice_del(z); /* delete, line 177 */
                if (ret < 0) return ret;
            }
            break;
        case 10:
            {   SWINT_T ret = slice_from_s(z, 1, s_18); /* <-, line 178 */
                if (ret < 0) return ret;
            }
            break;
        case 11:
            {   SWINT_T ret = slice_from_s(z, 1, s_19); /* <-, line 179 */
                if (ret < 0) return ret;
            }
            break;
        case 12:
            {   SWINT_T ret = slice_del(z); /* delete, line 180 */
                if (ret < 0) return ret;
            }
            break;
        case 13:
            {   SWINT_T ret = slice_del(z); /* delete, line 181 */
                if (ret < 0) return ret;
            }
            break;
        case 14:
            {   SWINT_T ret = slice_from_s(z, 1, s_20); /* <-, line 182 */
                if (ret < 0) return ret;
            }
            break;
        case 15:
            {   SWINT_T ret = slice_from_s(z, 1, s_21); /* <-, line 183 */
                if (ret < 0) return ret;
            }
            break;
        case 16:
            {   SWINT_T ret = slice_del(z); /* delete, line 184 */
                if (ret < 0) return ret;
            }
            break;
        case 17:
            {   SWINT_T ret = slice_del(z); /* delete, line 185 */
                if (ret < 0) return ret;
            }
            break;
        case 18:
            {   SWINT_T ret = slice_del(z); /* delete, line 186 */
                if (ret < 0) return ret;
            }
            break;
        case 19:
            {   SWINT_T ret = slice_from_s(z, 1, s_22); /* <-, line 187 */
                if (ret < 0) return ret;
            }
            break;
        case 20:
            {   SWINT_T ret = slice_from_s(z, 1, s_23); /* <-, line 188 */
                if (ret < 0) return ret;
            }
            break;
    }
...
static SWINT_T r_plur_owner(struct SN_env * z) {
    ...
    switch(among_var) {
        case 0: return 0;
        case 1:
            {   SWINT_T ret = slice_del(z); /* delete, line 194 */
                if (ret < 0) return ret;
            }
            break;
        case 2:
            {   SWINT_T ret = slice_from_s(z, 1, s_24); /* <-, line 195 */
                if (ret < 0) return ret;
            }
            break;
        case 3:
            {   SWINT_T ret = slice_from_s(z, 1, s_25); /* <-, line 196 */
                if (ret < 0) return ret;
            }
            break;
        case 4:
            {   SWINT_T ret = slice_del(z); /* delete, line 197 */
                if (ret < 0) return ret;
            }
            break;
        case 5:
            {   SWINT_T ret = slice_del(z); /* delete, line 198 */
                if (ret < 0) return ret;
            }
            break;
        case 6:
            {   SWINT_T ret = slice_del(z); /* delete, line 199 */
                if (ret < 0) return ret;
            }
            break;
        case 7:
            {   SWINT_T ret = slice_from_s(z, 1, s_26); /* <-, line 200 */
                if (ret < 0) return ret;
            }
            break;
        case 8:
            {   SWINT_T ret = slice_from_s(z, 1, s_27); /* <-, line 201 */
                if (ret < 0) return ret;
            }
            break;
        case 9:
            {   SWINT_T ret = slice_del(z); /* delete, line 202 */
                if (ret < 0) return ret;
            }
            break;
        case 10:
            {   SWINT_T ret = slice_del(z); /* delete, line 203 */
                if (ret < 0) return ret;
            }
            break;
        case 11:
            {   SWINT_T ret = slice_del(z); /* delete, line 204 */
                if (ret < 0) return ret;
            }
            break;
        case 12:
            {   SWINT_T ret = slice_from_s(z, 1, s_28); /* <-, line 205 */
                if (ret < 0) return ret;
            }
            break;
        case 13:
            {   SWINT_T ret = slice_from_s(z, 1, s_29); /* <-, line 206 */
                if (ret < 0) return ret;
            }
            break;
        case 14:
            {   SWINT_T ret = slice_del(z); /* delete, line 207 */
                if (ret < 0) return ret;
            }
            break;
        case 15:
            {   SWINT_T ret = slice_del(z); /* delete, line 208 */
                if (ret < 0) return ret;
            }
            break;
        case 16:
            {   SWINT_T ret = slice_del(z); /* delete, line 209 */
                if (ret < 0) return ret;
            }
            break;
        case 17:
            {   SWINT_T ret = slice_del(z); /* delete, line 210 */
                if (ret < 0) return ret;
            }
            break;
        case 18:
            {   SWINT_T ret = slice_from_s(z, 1, s_30); /* <-, line 211 */
                if (ret < 0) return ret;
            }
            break;
        case 19:
            {   SWINT_T ret = slice_from_s(z, 1, s_31); /* <-, line 212 */
                if (ret < 0) return ret;
            }
            break;
        case 20:
            {   SWINT_T ret = slice_del(z); /* delete, line 214 */
                if (ret < 0) return ret;
            }
            break;
        case 21:
            {   SWINT_T ret = slice_del(z); /* delete, line 215 */
                if (ret < 0) return ret;
            }
            break;
        case 22:
            {   SWINT_T ret = slice_from_s(z, 1, s_32); /* <-, line 216 */
                if (ret < 0) return ret;
            }
            break;
        case 23:
            {   SWINT_T ret = slice_from_s(z, 1, s_33); /* <-, line 217 */
                if (ret < 0) return ret;
            }
            break;
        case 24:
            {   SWINT_T ret = slice_del(z); /* delete, line 218 */
                if (ret < 0) return ret;
            }
            break;
        case 25:
            {   SWINT_T ret = slice_del(z); /* delete, line 219 */
                if (ret < 0) return ret;
            }
            break;
        case 26:
            {   SWINT_T ret = slice_del(z); /* delete, line 220 */
                if (ret < 0) return ret;
            }
            break;
        case 27:
            {   SWINT_T ret = slice_from_s(z, 1, s_34); /* <-, line 221 */
                if (ret < 0) return ret;
            }
            break;
        case 28:
            {   SWINT_T ret = slice_from_s(z, 1, s_35); /* <-, line 222 */
                if (ret < 0) return ret;
            }
            break;
        case 29:
            {   SWINT_T ret = slice_del(z); /* delete, line 223 */
                if (ret < 0) return ret;
            }
            break;
    }
...
extern SWINT_T hungarian_ISO_8859_1_stem(struct SN_env * z) {
    {   SWINT_T c1 = z->c; /* do, line 229 */
        {   SWINT_T ret = r_mark_regions(z);
    ...
    {   SWINT_T m2 = z->l - z->c; (void)m2; /* do, line 231 */
        {   SWINT_T ret = r_instrum(z);
    ...
    {   SWINT_T m3 = z->l - z->c; (void)m3; /* do, line 232 */
        {   SWINT_T ret = r_case(z);
    ...
    {   SWINT_T m4 = z->l - z->c; (void)m4; /* do, line 233 */
        {   SWINT_T ret = r_case_special(z);
    ...
    {   SWINT_T m5 = z->l - z->c; (void)m5; /* do, line 234 */
        {   SWINT_T ret = r_case_other(z);
    ...
    {   SWINT_T m6 = z->l - z->c; (void)m6; /* do, line 235 */
        {   SWINT_T ret = r_factive(z);
    ...
    {   SWINT_T m7 = z->l - z->c; (void)m7; /* do, line 236 */
        {   SWINT_T ret = r_owned(z);
    ...
    {   SWINT_T m8 = z->l - z->c; (void)m8; /* do, line 237 */
        {   SWINT_T ret = r_sing_owner(z);
    ...
    {   SWINT_T m9 = z->l - z->c; (void)m9; /* do, line 238 */
        {   SWINT_T ret = r_plur_owner(z);
    ...
    {   SWINT_T m10 = z->l - z->c; (void)m10; /* do, line 239 */
        {   SWINT_T ret = r_plural(z);
