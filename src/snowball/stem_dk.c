...
extern "C" {
#endif
extern SWINT_T danish_ISO_8859_1_stem(struct SN_env * z);
#ifdef __cplusplus
}
#endif
static SWINT_T r_undouble(struct SN_env * z);
static SWINT_T r_other_suffix(struct SN_env * z);
static SWINT_T r_consonant_pair(struct SN_env * z);
static SWINT_T r_main_suffix(struct SN_env * z);
...
static SWINT_T r_mark_regions(struct SN_env * z) {
    z->I[0] = z->l;
    {   SWINT_T c_test = z->c; /* test, line 33 */
        {   SWINT_T ret = z->c + 3;
    ...
    {    /* gopast */ /* non v, line 34 */
        SWINT_T ret = in_grouping(z, g_v, 97, 248, 1);
        if (ret < 0) return 0;
        z->c += ret;
    }
...
static SWINT_T r_main_suffix(struct SN_env * z) {
    SWINT_T among_var;
    {   SWINT_T mlimit; /* setlimit, line 41 */
    ...
    switch(among_var) {
        case 0: return 0;
        case 1:
            {   SWINT_T ret = slice_del(z); /* delete, line 48 */
                if (ret < 0) return ret;
            }
            break;
        case 2:
            if (in_grouping_b(z, g_s_ending, 97, 229, 0)) return 0;
            {   SWINT_T ret = slice_del(z); /* delete, line 50 */
                if (ret < 0) return ret;
            }
            break;
    }
...
static SWINT_T r_consonant_pair(struct SN_env * z) {
    {   SWINT_T m_test = z->l - z->c; /* test, line 55 */
        {   SWINT_T mlimit; /* setlimit, line 56 */
            SWINT_T m1 = z->l - z->c; (void)m1;
            if (z->c < z->I[0]) return 0;
...
static SWINT_T r_other_suffix(struct SN_env * z) {
    SWINT_T among_var;
    {   SWINT_T m1 = z->l - z->c; (void)m1; /* do, line 66 */
        z->ket = z->c; /* [, line 66 */
    ...
    {   SWINT_T mlimit; /* setlimit, line 67 */
    ...
    switch(among_var) {
        case 0: return 0;
        case 1:
            {   SWINT_T ret = slice_del(z); /* delete, line 70 */
                if (ret < 0) return ret;
            }
            {   SWINT_T m3 = z->l - z->c; (void)m3; /* do, line 70 */
                {   SWINT_T ret = r_consonant_pair(z);
            ...
            break;
        case 2:
            {   SWINT_T ret = slice_from_s(z, 3, s_2); /* <-, line 72 */
                if (ret < 0) return ret;
            }
            break;
    }
...
static SWINT_T r_undouble(struct SN_env * z) {
    {   SWINT_T mlimit; /* setlimit, line 76 */
        SWINT_T m1 = z->l - z->c; (void)m1;
        if (z->c < z->I[0]) return 0;
...
extern SWINT_T danish_ISO_8859_1_stem(struct SN_env * z) {
    ...
    {   SWINT_T m2 = z->l - z->c; (void)m2; /* do, line 86 */
        {   SWINT_T ret = r_main_suffix(z);
    ...
    {   SWINT_T m3 = z->l - z->c; (void)m3; /* do, line 87 */
        {   SWINT_T ret = r_consonant_pair(z);
    ...
    {   SWINT_T m4 = z->l - z->c; (void)m4; /* do, line 88 */
        {   SWINT_T ret = r_other_suffix(z);
    ...
    {   SWINT_T m5 = z->l - z->c; (void)m5; /* do, line 89 */
