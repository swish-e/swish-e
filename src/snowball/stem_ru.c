...
extern "C" {
#endif
extern SWINT_T russian_KOI8_R_stem(struct SN_env * z);
#ifdef __cplusplus
}
#endif
static SWINT_T r_tidy_up(struct SN_env * z);
static SWINT_T r_derivational(struct SN_env * z);
static SWINT_T r_noun(struct SN_env * z);
static SWINT_T r_verb(struct SN_env * z);
static SWINT_T r_reflexive(struct SN_env * z);
static SWINT_T r_adjectival(struct SN_env * z);
static SWINT_T r_adjective(struct SN_env * z);
static SWINT_T r_perfective_gerund(struct SN_env * z);
...
static SWINT_T r_mark_regions(struct SN_env * z) {
    z->I[0] = z->l;
    z->I[1] = z->l;
    {   SWINT_T c1 = z->c; /* do, line 63 */
        {    /* gopast */ /* grouping v, line 64 */
            SWINT_T ret = out_grouping(z, g_v, 192, 220, 1);
            if (ret < 0) goto lab0;
            z->c += ret;
        }
        z->I[0] = z->c; /* setmark pV, line 64 */
        {    /* gopast */ /* non v, line 64 */
            SWINT_T ret = in_grouping(z, g_v, 192, 220, 1);
            if (ret < 0) goto lab0;
            z->c += ret;
        }
...
static SWINT_T r_perfective_gerund(struct SN_env * z) {
    ...
    switch(among_var) {
        case 0: return 0;
        case 1:
            {   SWINT_T m1 = z->l - z->c; (void)m1; /* or, line 78 */
        ...
        lab0:
            ...
            break;
        case 2:
            {   SWINT_T ret = slice_del(z); /* delete, line 85 */
                if (ret < 0) return ret;
            }
            break;
    }
...
static SWINT_T r_adjectival(struct SN_env * z) {
    ...
    {   SWINT_T m_keep = z->l - z->c;/* (void) m_keep;*/ /* try, line 111 */
        ...
        switch(among_var) {
            ...
            case 1:
                {   SWINT_T m1 = z->l - z->c; (void)m1; /* or, line 117 */
            ...
            lab1:
                ...
                break;
            case 2:
                {   SWINT_T ret = slice_del(z); /* delete, line 124 */
                    if (ret < 0) return ret;
                }
                break;
        }
...
static SWINT_T r_verb(struct SN_env * z) {
    ...
    switch(among_var) {
        ...
        lab0:
            ...
            break;
        case 2:
            {   SWINT_T ret = slice_del(z); /* delete, line 153 */
                if (ret < 0) return ret;
            }
            break;
    }
...
static SWINT_T r_tidy_up(struct SN_env * z) {
    ...
    switch(among_var) {
        ...
        case 2:
            ...
            break;
        case 3:
            {   SWINT_T ret = slice_del(z); /* delete, line 196 */
                if (ret < 0) return ret;
            }
            break;
    }
...
extern SWINT_T russian_KOI8_R_stem(struct SN_env * z) {
    ...
    {   SWINT_T mlimit; /* setlimit, line 204 */
        ...
        {   SWINT_T m3 = z->l - z->c; (void)m3; /* do, line 205 */
            {   SWINT_T m4 = z->l - z->c; (void)m4; /* or, line 206 */
                {   SWINT_T ret = r_perfective_gerund(z);
            ...
            lab3:
                z->c = z->l - m4;
                {   SWINT_T m_keep = z->l - z->c;/* (void) m_keep;*/ /* try, line 207 */
                    {   SWINT_T ret = r_reflexive(z);
                ...
                {   SWINT_T m5 = z->l - z->c; (void)m5; /* or, line 208 */
                    {   SWINT_T ret = r_adjectival(z);
        ...
        {   SWINT_T m_keep = z->l - z->c;/* (void) m_keep;*/ /* try, line 211 */
            z->ket = z->c; /* [, line 211 */
            if (!(eq_s_b(z, 1, s_9))) { z->c = z->l - m_keep; goto lab8; }
            z->bra = z->c; /* ], line 211 */
            {   SWINT_T ret = slice_del(z); /* delete, line 211 */
                if (ret < 0) return ret;
            }
        lab8:
            ;
        }
        {   SWINT_T m6 = z->l - z->c; (void)m6; /* do, line 214 */
            {   SWINT_T ret = r_derivational(z);
        ...
        {   SWINT_T m7 = z->l - z->c; (void)m7; /* do, line 215 */
            {   SWINT_T ret = r_tidy_up(z);
