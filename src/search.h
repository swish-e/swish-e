...
typedef struct
{
    unsigned char   *inPropRange;  /* indexed by file number -- should be a vector to save room, but what is fastest?  SWINT_T? */
...
struct s_SEARCH_OBJECT
{
    ...
    SWINT_T             limits_prepared;   /* Flag that the parameters have been prepared */
...
//    SWINT_T         count;          /* result Entry-Counter */
    SWINT_T         filenum;        /* there's an extra four bytes we don't need */
    FileRec     fi;             /* This is used to cache the properties and the seek index */
    SWINT_T         rank;
    SWINT_T         frequency;
    SWINT_T         tfrequency;     /* Total frequency of result OR result index */
                                /* during result sorting tfrequency is used as an index number */
    /* proximity */
    SWINT_T         bArea;     
    SWINT_T         *pArea;

    SWUINT_T         posdata[1];     /* used for phrase searches */
};
...
typedef struct
{
    SWINT_T              direction;  /* -1 for asc and 1 for desc */
    ...
    SWINT_T              is_rank_sort; /* flag for faster sorting by rank */
} SortData;
...
struct s_DB_RESULTS
{
    ...
    SWINT_T          index_num;             /* index into params indexed by index number */
    ...
    SWINT_T          num_sort_props;        /* number of sort properties */
    ...
    SWINT_T          result_count;          /* number of results in set */
...
struct s_RESULTS_OBJECT
{
    ...
    SWINT_T             cur_rec_number;     /* current record number in list */
    
    SWINT_T             total_results;      /* total number of results */
    SWINT_T             total_files;        /* total number of files in all combined indexes */
    SWINT_T             search_words_found; /* flag that some search words were found in some index after parsing -- for error message */
    SWINT_T             lasterror;          /* used to save errors while processing more than one index file */
    SWINT_T             bigrank;            /* Largest rank found, for scaling */
    SWINT_T             rank_scale_factor;  /* for scaling each results rank when fetching with SwishNextResult */
...
void SwishRankScheme( SWISH *sw, SWINT_T scheme );	/* set ranking scheme */
...
SWINT_T isMetaNameOpNext(struct swline *);
