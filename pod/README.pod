...
This gives you the power to index and search your documents the way you wish and to seemlessly integrate
...
A list of platforms where Swish-e has been build can be found in the L<INSTALL|INSTALL> page.
...
The F<filters> directory contains Perl modules to convert documents from a format
that Swish-e cannot index directly (pdf) to something that Swish-e can index (HTML).
The modules simply make it easy to add filtering to external programs or to files
indexed when spidering a remote web site.

See the F<filters/README> file form more information.

=item filter-bin/

Sample programs to use with Swish-e's C<FileFilter> feature.  The C<FileFilter>
allows running an external program to filter or convert documents before indexing.
Examples include PDF, MS Word, and binary strings filters.
Filters often require installing separate document conversion programs (e.g. catdoc or pdftotext).
