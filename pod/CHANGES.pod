...
=item Change to default ParserWarnLevel

The default value for ParserWarnLevel was changed form zero to two.  This was to report
when libxml2 has problems parsing a document (which often times results in libxml2 processing
only part of a document).  To get the old behaviour set ParserWarnLevel to zero in your
config file.
