...
=item added -D prop_delimiter option

The new -D option at indexing time lets you specify an alternate char delimiter
when property values are appended to one another at indexing time. The default
value is the same as it always has been: a single space.
...
=item yanked SWISH-Stemmer-0.05.tar.gz

The tar file of SWISH::Stemmer in the example dir has been broken for years,
but since the module itself is severely deprecated we haven't bothered with it.
Till now. Because it was preventing the good people of the fedora project
from building their RPMs.

=item fix configure.in to require Berkeley DB

Reported on mailing list by Gorka Lertxundi Osa.

=item fix configure.in to require libxml2 and xml2-config

Reported on mailing list by Gorka Lertxundi Osa.

=item fix perl/Makefile.PL

r1974 merged from trunk.

=item New Berkeley DB backend

Jose Ruiz contributed a new BDB index format. The old native
and experimental incremental index formats have been removed.

=item Removed -S http feature

The spider.pl script is now preferred in combination with -S prog.

=item Removed expat parser

libxml2 parser is now required.
