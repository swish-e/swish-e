...
of a word: otherwise is is considered a normal character (i.e. can be
...
=head2 Single-Character Wildcard

The single-character wildcard character (?) will match any one character
in a word, except the first character.

Example:

 swish-e -w 's?ow'

will match C<snow> and C<show>.
