...
=head3 Why do I get an error about COALESCE_BUFFER_MAX_SIZE?

If you are indexing a document with a huge number of words (or perhaps a binary file)
you may see this message:

    err: Buffer too short in coalesce_word_locations.
    Increase COALESCE_BUFFER_MAX_SIZE in config.h and rebuild.

Check to make sure you not indexing a binary file.  If you are truly indexing
a text document with a huge number of words consider breaking up the document into
smaller chunks -- this will make search results more helpful.

Otherwise, you can follow the suggestion above to increase the
COALESCE_BUFFER_MAX_SIZE setting.
