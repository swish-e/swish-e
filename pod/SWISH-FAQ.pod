...
=head3 How does swish which parser to use?
...
will default to the HTML parser (but its document type -- used by a number of configuration
directives -- will be undefined).
...
C<IgnoreMetaTags> works with XML documents (and HTML documents when using libxml2 as the HTML parser),
...
This may or may not use less memory.
