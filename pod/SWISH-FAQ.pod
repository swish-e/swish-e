...
In general (for both spidering and the file system)
use the C<FileRules> directive to control what is indexed.  
...
=head2 How do I prevent indexing *parts* of a web page

If, for example, you want to keep swish from indexing a common header, footer, or navigation bar, AND
...
the C<ParserWarningLevel> is set.


=head2 I'm using the spider.pl program to spider my web site, but some files large files are not indexed.
...
=head2 How do I limit *searches* to just parts of the index?
...
after that upto, but not including the next "/" and save it in variable $1, and
then match everything from the "/" ownward.  Then replace the entire matches string
...
This has the added (yet small) advantage over indexing swishdocpath of saving
a little memory while indexing.
...
=head2 I'm using the prog method to index PDF documents, but the file contents are not indexed.
...
=head2 What are document properties?
...
=head2 What's the difference between MetaNames and PropertyNames?
