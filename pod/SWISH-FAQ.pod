...
Libxml2 is not required, but is recommended for parsing HTML documents.
...
The index file is currently made up of two files, one as specified by C<IndexFile> or
C<-f>, and a file of the same name but ends in F<.prop>.  This file contains the
document properties.
