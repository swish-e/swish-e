...
This only applies to HTML <meta name="...> tags if not using libxml2.

=item index - The contents of the meta tag are indexed, but placed in the main index unless there's
an enclosing metatag already in force. This is the default.

=item auto - This method create meta tags automatically for
...
=item disable - XML attributes are not parsed and not indexed.  This is the default.

=item error - If the concatenated meta name (e.g. person.ag) is not listed in B<MetaNames>
then indexing will be halted and an error reported.

=item ignore - The contents of the meta tag are ignored and B<not> indexed unless a metaname as
be defined with the C<MetaNames> directive.

=item index - The contents of the meta tag are indexed, but placed in the main index unless there's
an enclosing metatag already in force.

=item auto - This method will create meta tags automatically for
HTML meta names and XML elements.  Using this is the same as specifying all the meta names explicitly in a
B<MetaNames> dirictive.
