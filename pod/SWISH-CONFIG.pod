...
in quotes (single or double).  You may use a backslash to escape the next character.
...
enclose it in quotes of the other type.
    ...
    FileFilter .foo foofilter "'%p'"  <- unix
    FileFilter .foo foofilter \'%p\'  <- unix same thing
...
In case this seems a bit confusing, processing a directory happens in the following order.
Remember, directory names will have a trailing path delimiter when checking.
...
while indexing.  There is not default.
...
Swish uses the C<popen()> system call to run the external command, and the command is passed through
the shell.  For the standard F<command.com> shell this means that path names must use the backslash character
to separate path elements.  In addition, swish will make sure the program specified actually exists, which
...
The filter program is opened with C<popen()> which passes the command through the shell.
This means that paths to the filter program must be in DOS format and use the backslash as
the path separator.  Swish, like many shells, use the backslash as an escape character, so you will
need to specify the filter program like:
