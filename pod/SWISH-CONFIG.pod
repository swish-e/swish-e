...
0 to 4 - 0 is totally silent, 3 is the most verbose, while 4 and above will generate
debugging info. The default is 3, so you probably should define this.

    IndexReport 1

This may be overridden from the command line via the C<-v> switch (see L<SWISH-RUN|SWISH-RUN>).

IndexReport 4 will list all words indexed, so use caution when using this level.
