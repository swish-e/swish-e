...
enabling the library are described below.  Currently, this library is not required, but
is a much better parser than the tired old swish-e parsers html.c and xml.c  xml.c has been
replaced with James Clark's Expat library, but the html.c code is still broken in a number of ways.
Libxml2 is compatible with Expat, and offers a much better HTML parser.  Use it if possible.
...
=head2 Using libxml2 parser library
...
Installing the library is not required at this time, but is recommended, espeically
if you are running Linux or similar OS (e.g. Solaris) since installation is so easy.
...
installation is very similar to the way Swish-e is built.

If it complains that it can not find zlib then you may need to specify the location of
libz.  This happens (on Solaris) when the ./configure script finds the zlib header files,
...
In this case, try specifying where zlib can be found.

    ./configure --with-zlib=/usr/local

This would be used if libz was located in /usr/local/lib.

If you do not have root access, then you will need to set environment
variables to point to libxml2's library and header files *before* running
I<swihs-e's> configure script.  For example, say you want to link against
the library in its build directory:

    ~/swish-e > export CFLAGS="-I$HOME/libxml2-2.4.5/include -g -O2"
    ~/swish-e > export LDFLAGS="-R$HOME/libxml2-2.4.5/.libs -L$HOME/libxml2-2.4.5/.libs"
    ~/swish-e > ./configure

Under other shells (tcsh) you might use setenv:

    ~/swish-e > setenv CFLAGS "-I$HOME/libxml2-2.4.5/include -g -O2"
    ~/swish-e > setenv LDFLAGS "-R$HOME/libxml2-2.4.5/.libs -L$HOME/libxml2-2.4.5/.libs"
    ~/swish-e > ./configure

Then before running make you must edit the Makefile in the F<src> directory
...
Now, you run make as described in the next section.    


Note that libxml2 will be linked as a
shared library, so once you compile Swish-e to use the library, the library
...
see bug fix or security announcements, and a place where B<you> can offer help to others.

The list is typically very low traffic, so it won't overload your inbox.  Please take time
to subscribe.  See http://swish-e.org.

If you are using SWISH-E on a public site, please let the list know!
...
Other examples can be found in the F<conf> directory, and you may find the L<SWISH-FAQ|SWISH-FAQ>
helpful.
