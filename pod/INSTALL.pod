...
Build from source if your distribution does not offer the current version of swish-e.

A Windows binary version is available as a separate download from the Swish-e site
(http://swish-e.org).  Many of the installation instructions below will not apply to Windows
users. One exception is that the Perl LWP library is required for spidering remote site.
...
that swish-e can index.  These additional applications and tools must be installed
separately.
...
a number of platforms, including Sun/Solaris, BSD, Linux, OS X, and Open VMS.
...
operating system vendor, or install from source.

Some of these utilities or Perl modules are used for indexing 

=over 4

=item * Libxml2

Libxml2 is very stronly recommended.  It used used for parsing both HTML and XML files.
Swish-e can be built and installed without libxml2, but the HTML parser built into swish-e
is not as accurate as libxml2.

    http://xmlsoft.org/

For swish-e to use libxml2 it must be installed before building swish-e.

=item * Z Lib Compression

You should have zlib installed on your machine.  (If you don't your computer probably
runs on vacuum tubes).  zlib is used for compressing text stored in the swish-e index.
...
The LWP bundle of Perl modules is used for spidering web servers.  If you plan to index 
documents located on a web server you must have the LWP bundle of modules installed.
Most Perl installations include the LWP Perl modules.  Type "perldoc LWP" to see if
LWP is installed on your system.  If not, contact your system administrator or visit
http://search.cpan.org.  Every Perl installation should have the LWP library installed.

=item * PDF Documents

Indexing PDF files requires the xpdf package.  This is a common package available with
most operating systems and often provided as a package.

    http://www.foolabs.com/xpdf/

Xpdf may be added after swish-e is installed.

=item * MS Word Documents
...
Indexing MP3 ID3 Tags requires the MP3::Tag Perl module.  See http://search.cpan.org.
MP3::Tag may be installed after swish-e is installed.
...
These also may be installed after swish-e is installed.
...
The B<most> important section below describes how to get help on the swish-e
discussion list.  Please read and follow the suggestions as it will make it much
more likely that you will receive effective and timely help.
...
available and adjust below!
    ...
    ~/swish-e-2.4.0 $ make
    ~/swish-e-2.4.0 $ make check
    ...
    ==================
    All 3 tests passed
    ==================

    ~/swish-e-2.4.0 $ su root
    (password)
    ~/swish-e-2.4.0 # make install
    ~/swish-e-2.4.0 # exit
    ~/swish-e-2.4.0 $ swish-e -V
    SWISH-E 2.4.0

B<IMPORTANT:> Do not run swish-e as the superuser (root) -- root is only required
during the installation step when installing into system directories.

Here's another example.  This might be used if you do not have root access or to install
swish someplace other than /usr/local.

    ~ $ tar zxof swihs-e-2.4.0.tar.gz  (GNU tar with "z" option)
    ~ $ mkdir build && cd build  (with GNU make build is a separate build directory)
...
will display the options.  Two options are of common interest: --prefix sets the
installation directory, and --disable-shared will link swish-e statically, which may be
needed on some platforms (Solaris 2.6 perhaps).

    ~/build $ ./configure --prefix=$HOME/swish-e
    ~/build $ make >/dev/null  (only see warnings and errors)
    ~/build $ make check
    ...
    ==================
    All 3 tests passed
    ...
    ~/build $ make instal
    ~/build $ $HOME/swish-e/bin/swish-e -V 
    SWISH-E 2.4.0


=head2 What's installed

Swish installs a number of files.  By default all files are installed below /usr/local,
but this can be changed by setting --prefix when running configure (as shown above).  Individual
paths may also be set.  Run F<./configure --help> for details.


   $prefix/bin/swish-e         The swish-e binary program
   $prefix/share/doc/swish-e   Full Documentation
   $prefix/lib/libswish-e      The swish-e C library
   $prefix/include/swish-e.h   The library header file (for developers only)
   $prefix/man/                Documentation as manual pages
   $prefix/lib/swish-e         Helper programs (spider.pl, swishspider, swish.cgi)
   $prefix/lib/swish-e/perl    Perl helper modules


Note that the Perl modules are not installed in the system Perl library.
Swish-e and the Perl scripts that require the modules know where to find them, but
the F<perldoc> program used for reading documentation does not.  This can be corrected
by adding $prefix/lib/swish-e and $prefix/lib/swish-e/perl to the PERL5LIB environment
varilable.


=head1 BASIC USAGE

This sections gives a very basic overview of indexing with Swish-e.  
...
will index all files in the /var/www/htdocs directory.  You may also specify one or more 
files with the -i options.  Then to search for a give word:

     swish-e -w apache

Swish assumes files are HTML unless otherwise told so.  So if /var/www/htdocs contains 
non-HTML then you will need a configuration file.  Create a file called "swish.conf":
    ...
    # Tell swish that .txt files are to be parsed as text
...
The Swish-e configuration settings are described in the L<SWISH-CONFIG|SWISH-CONFIG> manual 
page.  The runtime options (switches) are described in the L<SWISH-RUN|SWISH-RUN> manual 
page.

Swish-e has two other methods reading input files.  One method uses a Perl helper script and 
the LWP Perl library to spider remove web sites:

    swish-e -S http -i http://localhost/index.html -v2

This will spider the web server running on the local host.  The C<-S> options defines in
input source to be "http", C<-i> specifies the URL to spider, and C<-v> sets the verbose
level to two.  There are a number of configuration options specific to the -S http input 
source.  See L<SWISH-CONFIG|SWISH-CONFIG>.  Note that only files of Content-Type text/* will 
be indexed.

The -S http method is depreciated in favor of the next input method.

The other method is a general purpose input method where swish reads input from a program 
...
The Swish-e distribution includes a spider program that uses this method of input.  This 
spider program is much more configurable and feature rich than the previous -S http method.
To duplicate the previous example create a configuration file called "swish2.conf"

    # Example for spidering
    # Use the "spider.pl" program
    IndexDir spider.pl
...
This says to use the -S prog input source method.  IndexDir in this case does not list a 
file or directory to index, but a program name run.  This program, spider.pl, does the work 
of fetching the documents from the web server and passing them to Swish-e for indexing.
...
The advantage of this method of spidering (over the previous "-S http" method) is that the 
Perl code is only compiled once instead of for every document fetched from the web server.  
In addition it is a much more advanced spider with many, many features.  Still, as used 
here, spider.pl will automatically index PDF or MS Word documents if (when) Xpdf and Catdoc 
are installed.
...
Swish-e creates a reverse index.  Just like an index in a book, you look up a word and
it lists the pages (or documents) where that word can be found.  Of course, Swish-e can do
boolean searches, phrase matches and wildcard searches, too.

Swish-e also can create multipe indexes at the same time.  For example, you might want to
create an index of just words in HTML titles in addition to an index of the words in the
body of the document.  Some database systems might call these different "fields" or
"columns", but swish-e calls them I<MetaNames> (as a result of first indexing HTML meta
tags).
...
Swish-e search results are a list of files -- actually file numbers.  Data can be associated 
with each file number when indexing.  For example, by default Swish-e associates the file's 
name, last modified data, and size with the file number and these items can be printed in 
search results.  In Swish-e this associated data is called a file's I<Properties>.  
Properties can be any data you wish to associated with a document -- even the entire 
text of the document can be stored in the index.  What data is stored as a Property is 
controlled by the I<PropertyNames> (and others) configuration directive.

And that's the basics of Swish-e.  The follow sections give more detailed exampls of using 
Swish-e.
...
been asked.  It's very important to provide the right information when asking
for help.  
...
For example, if you cannot find a document by a keyword that you believe should be indexed
try indexing just that single file, and use the -T INDEXED_WORDS options to see
if the word is actually being indexed.  First try without any changes to default
settings:
...
If that still isn't working as you expect try to reduce the test document to a very small
example.  This will be very helpful when asking for help.

Other tools are to use -H9 when searching to display full headers in search results.  Look 
at the "Parsed Words" header to see what words swish-e is searching for.
...
This is a more advanced example that spiders a web site using the included F<spider.pl>
program and uses the included F<example/swish.cgi> script for searching the index
via a web interface.
