...
enabling the library are described below.  Currently, the libxml2 library is not required, but
is a much better parser than the tired old swish-e parsers html.c and xml.c.
...
As mentioned above, the XML parser that swish ships with is fine.  The HTML parser that
ships with swish has been in use for years, but the parser provided by libxml2 is preferred.
...
If libxml2 complains during compilation that it can not find zlib then you may need to specify the location of
...
in /usr/local/lib you would use:

    ./configure --with-zlib=/usr/local

If you do not have root access and/or do not install libxml2 in the standard locations (for header
and library files), then you will need to set environment variables
to tell swish where to look for libxml2 files.  You will need to do this *before* running
I<swihs-e's> configure script since swish-e's configure script uses the settings to create
the Makefiles.

For example, say you want to link against the library in its build
directory of $HOME/libxml2-2.4.5.  You use CFLAGS to tell swish where to find the header files,
and use LDFLAGS to tell swish where to look for the library files when linking.
...
After running configure and before running make you must edit the Makefile in the F<src> directory
...
Now, you run C<make> as described in the following section.
...
Note: If you wish to use libxml2 (described in the previous section) you may need to
set CFLAGS and LDFLAGS before running F<./configure>.
