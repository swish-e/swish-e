...
Likewise, a setting of "XML" uses the included Expat library, where "XML"2
...
requires the LWP, HTTP, and HTML modules. (Note: depending on your perl
installation, you might need to install additional modules required
...
To enable Swish-e to use libxml2 you must run Swish-e's configure script as:
...
Note that libxml2 will be linked as a shared library, so once you
...
with libxml2 above.  To select support for zlib run configure with:
...
Again, as with compiling libxml2, you may need to munge the C<LD_RUN_PATH>
...
Make sure you are using the current stable release from
http://Swish-e.org.  How you download is up to you: lynx, lwp-download,
...
configuration file used during indexing.  You probably will not need to
change this file, but it's helpful to become familiar with the default
...
to be processed before they can be indexed.  Examples for using the C<-S
prog> method are shown in the F<prog-bin> directory>.

Swishe- can also use I<filters> to convert documents as they are
