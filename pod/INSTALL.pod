...
and the documentation in F</usr/local/share/swish-e>.
...
B<IMPORTANT:> Do not run swish-e as the superuser (root) -- this is only required
...
Then running "make install" will install the following files:
    ...
    $HOME/local/lib/libswish-e.a  -- the Swish-e search library
    $HOME/local/include           -- header file for the library
    $HOME/local/share/swish-e     -- documentation
...
=head2 The Swish-e C Search Library - libswish-e.a

Swish creates the C library F<libswish-e.a> during the build process.
Running "make install" installs the library and header file
in addition to the swish-e program.

By default the library in /usr/local/lib, but this
...
Searching an index will me many times faster when running under a
