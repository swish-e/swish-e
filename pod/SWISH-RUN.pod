...
=item -S [fs|http|prog] (indexing mode)
...
The C<prog> method is new to SWISH-E version 2.2.  It's meant as a general
...
The program name is passed to swish either by the C<IndexDir> directive,
or via the C<-i> option.
A few example programs are provided in the swish-e distribution
located in the C<prog-bin> directory.

The C<prog> method bypasses most of the configuration parameters available
to the file system method -- settings such as C<IndexOnly> and C<FileRules>
are ignored when using the C<prog> method.  This is because the external
program can filter documents before passing the document onto swish.  In
other words, when using the C<prog> method, only send the documents to swish
that you want indexed.

The C<http> method may be removed in the future since the C<prog> method
can supply this functionality
(an example web spider is provided in the prog-bin directory).
...
from enabling FollowSymLinks.

=item -N path (index only newer files)

The C<-N> option takes a path to a file, and only files I<newer> that the listed
