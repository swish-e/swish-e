...
When using swish-e on Windows, you must use the DOS-style backslash to separate the path segments.
This is required because swish runs the external program with popen() which uses the shell to run
the program and the DOS shell requires a backslash as the path separator.
...
This is because swish does some checks on the file name before running the program, and the
double-quotes within the file name will cause these checks to fail.  This may change in the future.
